...
```dockerfile
ENV NAME=world
CMD ["echo", "$NAME"]       # Выведет: $NAME (не интерполируется в командной строке!)
CMD echo $NAME              # Выведет: world (интерполируется shell'ом)
CMD exec echo $NAME         # Выведет: world (интерполируется shell'ом)
```

**Важно:** В exec форме переменные **доступны** приложению (через `os.environ`, `process.env`, etc), но **не подставляются** в саму команду Docker. Приложение должно само их прочитать из окружения контейнера.
