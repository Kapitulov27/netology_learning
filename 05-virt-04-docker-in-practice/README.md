...
2. Можете взять за образец представленный Dockerfile.
   Используйте базовый образ ```python:3.12-slim```.
   Обязательно используйте конструкцию ```COPY . .``` в Dockerfile. Не забудьте исключить ненужные в имадже файлы с помощью dockerignore.
   Используйте ```CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "5000"] ``` для запуска проекта.  
